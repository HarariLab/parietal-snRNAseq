...
m = subset(m,subset= nTREM2 != "TREM2" & Status == "NEURO_AD")

# apoe_table <- data.frame(as.data.frame.matrix(table(m@meta.data$Sample_ID, m@meta.data$nAPOE)))
...
m@meta.data$nAPOE = factor(m@meta.data$nAPOE, levels = c("33", "34"))
...
pred = model.matrix(~nAPOE + AOD,data=m@meta.data)

re_ln = nebula(GetAssayData(m,slot='counts'),m@meta.data$Sample_ID,pred=pred,method='LN',model='NBLMM')


tmp = data.frame(gene=re_ln$summary$gene, logFC_nAPOE34=re_ln$summary$logFC_nAPOE34)
tmp$z.score = re_ln$summary$logFC_nAPOE34/re_ln$summary$se_nAPOE34
tmp$p.value = pvalue.extreme(tmp$z.score)
tmp <- tmp[order(-abs(tmp$z.score)),]
tmp$BH = BH.extreme(tmp$z.score)
merged = merge(tmp,re_ln$summary,by=c('gene', "logFC_nAPOE34"))
rownames(merged) = merged$gene
merged = merged[order(abs(merged$z.score),decreasing=T),c(2:14,1)]


dir <- paste0('/home/bnovotny/single_nuclei/nebula_67/', celltype, '/nebula_de/DE_by_status/APOE/')
mainfilename <- paste('nebula_compareAPOE4_fixed', celltype, subclust, sep='_')
