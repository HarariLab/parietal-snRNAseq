...
m = readRDS(paste0('/home/brasel/SingleCellProjects/dataObjects/neuron.rds'))
pheno = read.csv('/home/brasel/SingleCellProjects/Brain_pheno_jorge.csv', stringsAsFactors = F)

excit = c(0,1,2,6)
inhib = c(3,4,5)

# Filter excitatory or inhibitory if needed
if(subclust == "excit"){ 
  m = subset(m,subset=SCT_snn_res.0.1 %in% excit)
}else if (subclust == "inhib") {
  m = subset(m,subset=SCT_snn_res.0.1 %in% inhib)
}

# Could specify status in arguments
reduced_variants <- c("H157Y", "R47H", "R62H")
TREM2_IDs <- pheno$Sample[pheno$TREM2_type %in% reduced_variants] # 13 samples with reduced variants (any status)

m = subset(m,subset= Sample_ID %in% TREM2_IDs | nTREM2 == "CO" )

m@meta.data$nTREM2 = factor(m@meta.data$nTREM2, levels = c("CO", "TREM2"))

m = subset(m,subset=Sample_ID %in% names(table(m@meta.data$Sample_ID)[table(m@meta.data$Sample_ID) > 60]))
...
if (!subclust %in% c('excit', 'inhib', 'all')){
  # 1 vs 1
...
pred = model.matrix(~nTREM2 + Gender + AOD,data=m@meta.data)

re_ln = nebula(GetAssayData(m,slot='counts'),m@meta.data$Sample_ID,pred=pred,method='LN',model='NBLMM')


tmp = data.frame(gene=re_ln$summary$gene, logFC_nTREM2TREM2=re_ln$summary$logFC_nTREM2TREM2)
tmp$z.score = re_ln$summary$logFC_nTREM2TREM2/re_ln$summary$se_nTREM2TREM2
tmp$p.value = pvalue.extreme(tmp$z.score)
tmp <- tmp[order(-abs(tmp$z.score)),]
tmp$BH = BH.extreme(tmp$z.score)
merged = merge(tmp,re_ln$summary,by=c('gene', "logFC_nTREM2TREM2"))
rownames(merged) = merged$gene
merged = merged[order(abs(merged$z.score),decreasing=T),c(2:14,1)]


dir <- paste0('/home/bnovotny/single_nuclei/', celltype, '/nebula_de/DE_by_status/TREM2vsCO/')
mainfilename <- paste('nebula_compareStatus_TREM2fixed', celltype, subclust, sep='_')
