...
m = readRDS(paste0('/home/brasel/SingleCellProjects/dataObjects/neurons_2_3_removed_12.18.20.rds'))
excit = c(0,1,2,6)
inhib = c(3,4,5)
...
m = subset(m,subset=Status %in% c('NEURO_AD','NEURO_CO') )
m = subset(m,subset=nTREM2 != "TREM2" )
m@meta.data$Status = factor(m@meta.data$Status, levels = c("NEURO_CO", "NEURO_AD"))
...
pred = model.matrix(~Status + Gender + AOD,data=m@meta.data)

re_ln = nebula(GetAssayData(m,slot='counts'),m@meta.data$Sample_ID,pred=pred,method='LN',model='NBLMM')


tmp = data.frame(gene=re_ln$summary$gene, logFC_StatusNEURO_AD=re_ln$summary$logFC_StatusNEURO_AD)
tmp$z.score = re_ln$summary$logFC_StatusNEURO_AD/re_ln$summary$se_StatusNEURO_AD
tmp$p.value = pvalue.extreme(tmp$z.score)
tmp <- tmp[order(-abs(tmp$z.score)),]
tmp$BH = BH.extreme(tmp$z.score)
merged = merge(tmp,re_ln$summary,by=c('gene', "logFC_StatusNEURO_AD"))
rownames(merged) = merged$gene
merged = merged[order(abs(merged$z.score),decreasing=T),c(2:14,1)]


dir <- paste0('/home/bnovotny/single_nuclei/', celltype, '/nebula_de/DE_by_status/sADvsCO/')
mainfilename <- paste('nebula_compareStatus_sAD_noTREM2', celltype, subclust, sep='_')
