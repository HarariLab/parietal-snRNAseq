...
m = readRDS('/home/brasel/SingleCellProjects/dataObjects/neurons_2_3_removed_12.18.20.rds')
excit = c(0,1,2,6)
inhib = c(3,4,5)
if(subclust1 %in% excit){ 
...
#re_ln = c()
#mainfilename <- paste('nebula_compareSubclusters_neuron', subclust1, subclust2,'01.13.21', sep='_')
#re_ln$summary = read.table(paste0(mainfilename,'.txt'),sep='\t',row.names=1)

tmp = data.frame(logFC_cluster=re_ln$summary$logFC_cluster)
...
merged = merge(tmp,re_ln$summary,by='logFC_cluster')
rownames(merged) = merged$gene
merged = merged[order(abs(merged$z.score),decreasing=T),]

dir <- '/home/bnovotny/single_nuclei/neuron/nebula_de/'
mainfilename <- paste('nebula_compareSubclusters_neuron', subclust1, subclust2,date, sep='_')
