...
if (subclust2 != 'all'){
  ...
  ## Use this to compare to only 0-4
  clusterSet[clusterSet != subclust1 & !clusterSet %in% c(0,1,2,3,4)] = 9
  clusterSet[clusterSet == subclust1] = -1
  clusterSet[clusterSet %in% c(0,1,2,3,4)] = 0
  clusterSet[clusterSet == -1] = 1
...
dir <- '/home/bnovotny/single_nuclei/microglia/nebula_de/'
# mainfilename <- paste('nebula_compareSubclusters_microglia', subclust1, subclust2,date, sep='_')
mainfilename <- paste('nebula_compareSubclusters_microglia', subclust1, "01234",date, sep='_')
