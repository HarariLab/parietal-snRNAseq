...
colnames(m)[grep(paste('neuron',c(0:2,6),sep='.',collapse='|'),colnames(m))] = paste('n_ex',c(0:3),sep='.')
colnames(m)[grep(paste('neuron',c(3:5),sep='.',collapse='|'),colnames(m))] = paste('n_inh',c(0:2),sep='.')
m[,colnames(m)[grep('n_ex',colnames(m))]] = m[,colnames(m)[grep('n_ex',colnames(m))]]/rowSums(m[,colnames(m)[grep('n_ex',colnames(m))]])
m[,colnames(m)[grep('n_inh',colnames(m))]] = m[,colnames(m)[grep('n_inh',colnames(m))]]/rowSums(m[,colnames(m)[grep('n_inh',colnames(m))]])
...
myFullDF = NULL
tmp = lapply(c('astro','micro','n_ex','n_inh','oligo','opc'),function(cellType){
...
write.table(myFullDF, '/home/brasel/SingleCellProjects/MyProjects/67BrainsPaper/createSourceFiles_NatComm/SupFigs/SupFig12.csv', sep = ', ', quote = F, row.names = F, col.names = T) # nolint: line_length_linter.
