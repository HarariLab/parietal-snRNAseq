...
myFullDF = NULL
tmp = lapply(c('micro','oligo'),function(cellType){
	...
	p = ggplot(sDF,aes(x=factor(Group.1,levels=c(str_wrap('TREM2 Reduced',width=10),'Other')),y=Proportion,fill=Group.2)) + geom_bar(position="stack", stat="identity")+ # for trem2_reduced plots
...
write.table(myFullDF, '/home/brasel/SingleCellProjects/MyProjects/67BrainsPaper/createSourceFiles_NatComm/fig4ae.csv', sep = ', ', quote = F, row.names = F, col.names = T)
