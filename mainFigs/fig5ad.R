...
myFullDF = NULL
tmp = lapply(c('micro','astro'),function(cellType){
    ...
    covar = 'rs1582763'
		...
		p = ggplot(sDF,aes(x=factor(Group.1,levels=c('GG','AG','AA')),y=Proportion,fill=Group.2)) + geom_bar(position="stack", stat="identity")+ # rs1582763 plots scale_fill_manual(values=c(brewer.pal(9, 'Set1')[1:length(unique(sDF[,2]))])) +
...
write.table(myFullDF, '/home/brasel/SingleCellProjects/MyProjects/67BrainsPaper/createSourceFiles_NatComm/fig5ad.csv', sep = ', ', quote = F, row.names = F, col.names = T)
